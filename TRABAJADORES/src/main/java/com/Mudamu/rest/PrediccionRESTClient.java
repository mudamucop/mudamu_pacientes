...
public class PrediccionRESTClient {
	...
	public Predicciones getPredicciones(Optional<Medico> user) {
		Predicciones predicciones = new Predicciones();
		WebResource webResource = client.resource(urlDDBBService).path("predicciones").queryParam("username",Integer.toString(user.get().getTrabajadorID()));
